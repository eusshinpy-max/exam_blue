...
def get_product_prices(product_name: str):
    ...
    try:
        ...
        latest_price_response = supabase_client.from_('농산물가격').select('평균가격').eq('품목', product_name).order('날짜', desc=True).limit(1).execute()
        ...
        previous_day_price_response = supabase_client.from_('농산물가격').select('평균가격').eq('품목', product_name).eq('날짜', yesterday).execute()
